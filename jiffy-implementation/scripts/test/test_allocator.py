...
class TestAllocator:
    ...
    def test_redist_thresh(self):
        ...
        alloc = Allocator(demands, total_blocks=total_blocks, init_credits=6, public_blocks=public_blocks, redistribution_thresh=public_blocks, inflation=3)
        allocations = alloc.compute()
        basic_checks(demands, allocations)
        for e in range(num_epochs):
            total_allocation = 0
        ...
        assert alloc.credit_map['A'] == 12
        assert alloc.credit_map['B'] == 12
        assert alloc.credit_map['C'] == 21
